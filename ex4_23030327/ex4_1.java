...
public class ex4_1 extends Application {
    // 数据库连接信息
    ...
    private void showLoginScreen(Stage stage) {
        ...
        titleLabel.setStyle("-fx-font-size: 18; -fx-font-weight: bold;"); //表示字体大小为18且字体加粗
    ...
    private void showMainScreen(Stage stage) {
        try {
            ...
            topContainer.getChildren().addAll(formPane, buttonBox);

            // 3. 创建表格 (这部分代码逻辑不变)
            createTableView();

            // 4. 将顶部容器和表格，依次添加到根布局VBox中
            root.getChildren().addAll(topContainer, studentTable);

            // 5.让表格在窗口拉伸时，自动填充所有可用的垂直空间
            VBox.setVgrow(studentTable, Priority.ALWAYS);

            // 6. 设置场景和舞台 (现在，即使设置了固定高度，VBox也能正确处理)
    ...
    private boolean validateInput() {
        ...
        // 验证家庭电话格式
        String mobile = mobilePhoneField.getText().trim();
        if (!mobile.isEmpty()) {
            // 定义手机号的正则表达式：以1开头，后跟10位数字
            String phoneRegex = "^1\\d{10}$";
            if (!mobile.matches(phoneRegex)) {
                showAlert("错误", "手机号码格式不正确！\n必须是1开头的11位数字。");
                return false;
            }
        }
        return true;
